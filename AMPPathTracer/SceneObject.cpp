...
Intersection SceneObject::TraceSphere(Ray r) restrict(amp, cpu)
{
	...
	Vector3 rOrigin = r.Origin - Origin;

	// Get A, B and C of quadratic
	float a = r.Direction.Dot(r.Direction);
	float b = 2 * rOrigin.Dot(r.Direction);
	float c = rOrigin.Dot(rOrigin) - (Radius * Radius);
	...
	if (t0 >= 0)
	{
		...
		return Intersection(tpos, (tpos - Origin).Normalize(), *this);
	}
...
Intersection SceneObject::TracePlane(Ray r) restrict(amp, cpu)
{
	...
	if (denom > 0.0f)
	{
		Vector3 dir = r.Origin - Origin;
		float t = dir.Dot(Normal) / denom;

		return Intersection(r.Origin + r.Direction * t, Normal, *this);
	}
